...
print("PSI_THRESHOLD:", PSI_THRESHOLD)
...
def lambda_handler(event, context):
    response = s3.get_object(Bucket=S3_BUCKET_OUTPUT, Key=S3_PSI_KEY)
    ...
    if not violations:
        message = "✅ No model drift detected. All PSI values are within threshold."
        ...
        writer.writerow(["name", "psi", "date"])
        for v in violations:
            table += f"| {v['name']} | {v['psi']} | {v['date']} |\n"
        ...
        message = f"""⚠️ **Model Drift Detected!**\n\n
                   ...
                   The following features have PSI > {PSI_THRESHOLD} after {SART_DATE}:\n\n
                   {table}"""
